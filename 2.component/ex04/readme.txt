...
01. 기본 개념
    1) State
        ...
        - 함수 컴포넌트에서는 상태 변경을 위해 userState 후크 함수에서 반환된 배열의 두번재 요소를 함수로 사용한다.

    2) 예제 src/01
02. 제어 컴포넌트
    1)  input, textarea, option과 같은 폼(<form/>) 컴포넌트에서 사용자 입력에 따라 state값이 변경되고 렌더링하는 컴포넌트를
    4)  제어 컴포넌트: 
        - 예제 src/02
...
03. 상태 컴포넌트 vs 순수(Pure, Dumb) 컴포넌트
    ...
    * 3. 애플리케이션의 컴포넌트들은 상태 컴포넌트와 순수 컴포넌트로 분리하여 개발하는 것이 좋다.
    4. 어떤 컴포넌트가 상태 컴포넌트인가?
        ...
        - 컴포넌트 Hierarchy에서 상위에 있는 상태를 가져야만 하는 컴포넌트
        - 못 찾는다면, 상태를 관리하는 컴포넌트를 만들고 하위(pure) 컴포넌트를 래핑하도록 한다.
